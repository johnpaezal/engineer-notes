# Software Engineering Notes

## Index

### 01 - Process
*Cómo se trabaja en equipo*

| # | Topic | Path |
|---|-------|------|
| 1 | Software Development Lifecycle | `01-process/sdlc.md` |
| 2 | Requirements & User Stories | `01-process/requirements.md` |
| 3 | Scrum & Agile | `01-process/scrum.md` |

### 02 - Design
*Diseño de código y APIs*

| # | Topic | Path |
|---|-------|------|
| 3 | REST API Design | `02-design/rest-api.md` |
| 4 | HTTP API Deep | `02-design/http-api-deep.md` |
| 5 | Design Patterns + MVC | `02-design/design-patterns.md` |
| 6 | Clean Code | `02-design/clean-code.md` |

### 03 - Architecture
*Estructura del sistema*

| # | Topic | Path |
|---|-------|------|
| 6 | Styles | `03-architecture/styles.md` |
| 7 | Components | `03-architecture/components.md` |
| 8 | Diagrams | `03-architecture/diagrams.md` |
| 9 | Patterns | `03-architecture/patterns.md` |
| 10 | Scalability | `03-architecture/scalability.md` |
| 11 | Prototipos (capas, monolito) | `03-architecture/tipos-arquitecturas/` |

### 04 - Quality
*Calidad, seguridad y confiabilidad*

| # | Topic | Path |
|---|-------|------|
| 12 | Testing | `04-quality/testing.md` |
| 13 | Security – OWASP | `04-quality/security-owasp.md` |
| 14 | Observability | `04-quality/observability.md` |
| 15 | Production Troubleshooting | `04-quality/troubleshooting.md` |

### 05 - Delivery
*Entrega y operación*

| # | Topic | Path |
|---|-------|------|
| 15 | DevOps & CI/CD | `05-delivery/devops-cicd.md` |
| 16 | IaC & Terraform | `05-delivery/iac-terraform.md` |
| 17 | Git Workflow | `05-delivery/git-workflow.md` |

### 06 - Communication
*Cómo hablan sistemas y equipos*

| # | Topic | Path |
|---|-------|------|
| 18 | Message Queues | `06-communication/message-queues.md` |
| 19 | Documentation | `06-communication/documentation.md` |

### 08 - Algorithms
*Algoritmia y estructuras de datos para entrevistas*

| # | Topic | Path |
|---|-------|------|
| 20 | Complexity (Big-O) | `08-algorithms/01-complexity.md` |
| 21 | Core Structures | `08-algorithms/02-core-structures.md` |
| 22 | Trees & Graphs | `08-algorithms/03-trees-graphs.md` |
| 23 | Sorting & Searching | `08-algorithms/04-sorting-searching.md` |
| 24 | Interview Patterns | `08-algorithms/05-patterns.md` |
| 25 | Cheat Sheet | `08-algorithms/06-cheat-sheet.md` |
